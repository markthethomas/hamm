...
func testStorage(t testing.TB) *Storage {
	fh, err := ioutil.TempDir("", "hamm")
	if err != nil {
		t.Fatalf("err: %s", err)
	}
	store, err := New(Options{Path: fh})
	if err != nil {
		t.Fatalf("err: %s", err)
	}
	return store
...
func TestStorage_FirstIndex(t *testing.T) {
	store := testStorage(t)
	defer store.Close()
...
func TestStorage_LastIndex(t *testing.T) {
	store := testStorage(t)
	defer store.Close()
	defer os.Remove(store.path)
	...
	if idx != 3 {
		t.Fatalf("bad final index, got: %d", idx)
	}
}

func TestStorage_GetLog(t *testing.T) {
	store := testStorage(t)
	defer os.Remove(store.path)
	defer store.Close()
...
func TestStorage_SetLog(t *testing.T) {
	store := testStorage(t)
	defer store.Close()
	defer os.Remove(store.path)
...
func TestStorage_SetLogs(t *testing.T) {
	store := testStorage(t)
	defer store.Close()
	defer os.Remove(store.path)
...
func TestStorage_DeleteRange(t *testing.T) {
	store := testStorage(t)
	defer store.Close()
	defer os.Remove(store.path)
...
func TestStorage_Set_Get(t *testing.T) {
	store := testStorage(t)
	defer store.Close()
	defer os.Remove(store.path)
...
func TestStorage_SetUint64_GetUint64(t *testing.T) {
	store := testStorage(t)
	defer store.Close()
	defer os.Remove(store.path)
